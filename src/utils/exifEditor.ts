 ...
 * 返回被删除的字段位置列表
 */
export function removeTagsFromIfd(
  ifd: ParsedIfd | null,
  tagsToRemove: number[],
...
export function setDateTimeValue(parsed: ParsedExif, dateTime: string, modifiedFields: FieldLocation[]): void {
 ...
 */
export function rebuildExifPayload(parsed: ParsedExif, removedFields: FieldLocation[]): Uint8Array {
...
/** PNG 8 字节签名（导出供 exifEditorPng.ts 使用） */
// --- PNG 相关（完整实现已拆分到 exifEditorPng.ts，按需动态加载）---
// 保留：PNG_SIGNATURE / isPngFile / PngTimeEntry / parseTimeChunk / formatPngTime
// 保留：HexDumpLine / HexDumpResult / HEX_DUMP_MAX_BYTES / formatHexDump（PNG/WebP 共用）
...
// ============================================================
// WebP 文件类型识别（轻量级，仅供文件类型检测使用）
// 完整的 WebP 元数据编辑逻辑已拆分至 exifEditorWebp.ts，按需动态加载
// ============================================================

/** RIFF 容器魔数（'RIFF' 4 字节） */
export const WEBP_RIFF_MAGIC = [0x52, 0x49, 0x46, 0x46]; // 'RIFF'
/** WebP 类型标识（'WEBP' 4 字节，位于文件头偏移 8） */
export const WEBP_TYPE_MAGIC = [0x57, 0x45, 0x42, 0x50]; // 'WEBP'
