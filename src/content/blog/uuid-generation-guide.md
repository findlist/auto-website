...
使用密码学安全的随机数生成器生成 122 位随机位。这是**最常用的版本**，实现简单，无需协调，隐私友好。本文重点讨论。若需从有限集合中随机抽取若干项（如抽奖、随机分组、A/B 测试分配），可使用 [随机抽取分配工具](/random-picker)。
...
> 若场景是「从有限集合中随机抽取若干项」（抽奖、随机分组、A/B 测试分配），用 [列表元素随机抽样工具](/random-picker) 比 UUID 更直接，无需协调且支持不重复抽样。
