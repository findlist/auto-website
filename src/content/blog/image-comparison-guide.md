...
- 对比两张构图相近但视口不同的截图时，先用[对比区域裁剪器](/image-crop)按相同比例裁剪到公共区域，再做像素级对比，可显著降低因画面位移引入的伪差异比例。
- 对比 PNG 与 WebP / JPEG 等不同格式输出时，先用[对比前格式统一转换器](/image-convert)将两侧统一为相同格式（推荐 PNG 无损），避免格式压缩差异污染"编码质量差异"评估结果。
- 对比加水印前后的图片时，先用[水印效果对比预览工具](/image-watermark)生成水印版本，再用对比工具量化水印对原图视觉质量的影响（如水印覆盖区域的差异比例）。
