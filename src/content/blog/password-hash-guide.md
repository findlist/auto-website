...
密码哈希是用户认证系统的基石——它将明文密码（推荐用 [高强度随机密码生成](/password) 产生）转换为不可逆的固定长度字符串，即使数据库泄露，攻击者也无法直接还原密码。但密码哈希与普通哈希（如 SHA-256 用于文件校验）有本质区别。
...
- **[待哈希强密码生成器](/password)**：生成强密码 → 用本工具哈希存储
