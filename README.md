...
共 **71 个在线开发工具** + **66 篇配套技术博客**，覆盖编码转换、加密哈希、时间日期、代码调试、网络计算、色彩设计、文档处理等常用场景。
...
- 📝 **配套博客** — 66 篇深度技术博客（290+ 个标签），覆盖工具原理、最佳实践、安全指南
...
### 色彩与设计
`颜色值转换` · `调色板工具` · `色彩对比度检测（WCAG）` · `CSS 盒阴影生成器` · `CSS 渐变生成器` · `CSS 文字阴影生成器` · `CSS border-radius 生成器` · `CSS transform 可视化工具` · `CSS 滤镜生成器`
...
| 交互层 | React 18 + TypeScript 5.7 | 71 个 React 工具组件（`@astrojs/react`） |
...
├── src/
│   ├── components/                # 71 个 React 工具组件（AesTool / Base64Tool / …）
│   ├── content/blog/              # 66 篇技术博客（.md）
│   ├── layouts/
...
每包含工具页均配独立 `title / description / OG meta / JSON-LD`，并纳入 sitemap 与 RSS。主要内容页面数量 420 页（含 66 篇博客 + 290+ 个标签筛选页 + 71 个工具页及其它）。
...
- **检查范围**：
  ...
  - 审查 `src/components/` 工具组件（JsonTool / JwtTool / RegexTool / HashTool 等全部 71 个工具组件）
