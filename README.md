...
本项目采用 [自主进化 Agent 规范 v2.0](./auto-site-spec.md) 维护，当前为已上线工具站的数据与搜索信号驱动维护期。

- 默认每天只读评估一次，无新增信号时直接跳过
- 信号来源：线上错误、CI、Search Console、访问统计、性能和用户反馈
- 单次最多实施一个达到评分门槛的低风险任务
- 没有合格任务时零修改、零提交结束
- 新工具、统计、广告、隐私、依赖和大规模 SEO 变化必须获得用户授权
- 风险 ≤1、证据充分、工作区干净且完整检查全绿时自动单任务推送并部署
- 禁止用 SEO 文案轮换、模块拆分和轮次文档填充产出
...
```text
你是 auto-website 的自主进化 Agent。完整读取并严格执行：
e:\work\auto-website\auto-site-spec.md

项目已上线，当前属于数据与搜索信号驱动维护期。只依据线上错误、测试或构建失败、Search Console、访问统计、Core Web Vitals、Bundle 体积、用户反馈和明确批准的任务产生候选。

只分析上次评估后变化的信号，最多列出 5 个候选并按规范评分。单次最多实施一个达到门槛的低风险任务。没有新增部署、告警、指标变化或授权任务时，必须零修改、零提交结束。

禁止无数据支撑地反复修改 title/description、结构化数据、拆分模块、抽取常量、新增工具或补写轮次文档。新增工具、计算语义、统计、广告、隐私、依赖、导航和大规模 SEO 改写必须等待用户授权。

修改前定义用户影响和验收标准，运行相关测试与构建。达到评分门槛、风险 ≤1、工作区干净，且 npm run check、完整生产构建和相关测试全部通过时，精确暂存本任务文件，自动创建一个提交并执行 git push origin HEAD 触发部署；否则不得提交。单次最多推送一个提交，push 失败不得追加提交。最后按规范输出精简评估摘要。
```
